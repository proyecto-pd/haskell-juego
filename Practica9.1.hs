...
jugada :: Tablero -> Int -> Int -> Tablero
jugada t f n = [if f /= i then x else x-n | (x,i)<-zip t [1..]]
...
escribeTablero :: Tablero -> IO ()
escribeTablero t = sequence_ [escribeFila f n | (f,n)<-zip [1..] t] 
...
juego t j = do
    ...
    putStrLn ("\n\nJ " ++ (show j))
    f<- leeDigito "\nElije una fila: "
    n<- leeDigito "\nElije cuantas estrellas retiras: "
    if valida t f n then do
        let nt = jugada t f n 
        if finalizado nt then do
